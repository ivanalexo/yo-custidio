...
export class ImageProcessingService {
    ...
    constructor(private configService: ConfigService) {
        this.imageProcessorUrl = this.configService.get<string>('app.imageProcessor.url') ?? 'http://localhost:5000/';
    }
    ...
    async processImage(imageBuffer: Buffer): Promise<{
        ...
        try {
            ...
            this.logger.log(`Decodificando imagen procesada, hash: ${result.imageHash}`);
            ...
            if (error instanceof Error) {
              ...
              throw new Error('Error procesando: ', error.message || JSON.stringify(error));
            }
        }
    }
